...
export default function IntroductionSection({ name, roles }) {
    return (
        <section id="home" className="flex flex-col-reverse lg:flex-row text-special-white bg-dark-blue">
            <section className="flex flex-col justify-center gap-5 lg:px-[5rem] px-8 py-12">
                ...
                <section className="flex gap-3">
                    { roles.map((role, index) => {
                        <div className="even:text-black bg-neon-blue odd:bg-neon-violet text-special-white text-sm font-bold uppercase px-3 py-2 rounded-md" key={index}>{ role }</div> 
                    })}
                </section>
            ...
            <section className="lg:h-screen lg:min-w-[50%] border-b-4 border-neon-violet lg:border-l-4 lg:border-neon-cyan">
